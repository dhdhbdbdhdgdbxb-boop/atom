...
async function getProductData(slug) {
  try {
    console.log('=== SERVER SIDE PRODUCT FETCH ===');
    ...
    console.log('Server side product found:', !!product);
    return product;
  } catch (error) {
...
// Генерация метаданных для SEO (основная английская версия)
export async function generateMetadata({ params }) {
  ...
  // Получаем переводы продукта (приоритет английскому для основной версии)
  ...
  const seoTitle = metaTitle || `${productName} - ${gameName} ${categoryName} Cheat | AtomCheats`;
...
export default async function ProductPage({ params }) {
  console.log('=== PRODUCT PAGE SERVER ===');
  ...
  if (!slug) {
    console.error('No slug in params after resolution!');
    notFound();
  }
