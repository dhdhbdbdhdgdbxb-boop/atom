...
// Мемоизированный компонент карточки для оптимизации рендеринга
const CatalogCard = memo(({ item, currentLevel, currentContent, onCardClick, getTypeIcon, getResultTypeLabel, getCheatStatusColor, getCheatStatusDotColor, getCheatStatusText }) => {
  return (
    <div
      key={item.id}
    ...
    >
      ...
      {currentLevel === 'search' && (
        <div className="absolute top-4 left-4 bg-cyan-500/80 text-white text-xs px-2 py-1 rounded-lg z-10">
          {getTypeIcon(item.type)}
        </div>
      )}
       
      {/* Product main image as background - only for products */}
      {item.type === 'product' && item.media && Array.isArray(item.media) && item.media.filter(m => m.isMainImage).length > 0 ? (
        <>
          <div
            className="absolute inset-0 z-0"
            style={{
              backgroundImage: `url(${item.media.filter(m => m.isMainImage)[0].url || item.backgroundImage || '/images/backgrounds/grid.png'})`,
              backgroundSize: 'cover',
              backgroundPosition: 'center',
              backgroundRepeat: 'no-repeat'
            }}
          />
          {/* Оверлей с градиентом для товаров */}
          <div className="absolute inset-0 bg-gradient-to-t from-[#10182C]/100 via-[#10182C]/40 to-transparent rounded-2xl z-5" />
        </>
      ) : (
        <div
          className="absolute inset-0 z-0"
      ...
      {/* Стандартный градиент для игр и категорий */}
      {item.type !== 'product' && (
        <div className="absolute inset-0 bg-gradient-to-t from-slate-900/80 via-slate-900/20 to-transparent rounded-2xl z-5" />
      )}
        
      {/* Cheat status badge - only for products with cheatStatus */}
      {item.type === 'product' && item.cheatStatus && (
        <div className={`absolute top-4 left-4 px-2 py-1 rounded-lg text-xs font-medium ${getCheatStatusColor(item.cheatStatus)} z-10 flex items-center gap-1`}>
          <div className={`w-2 h-2 rounded-full ${getCheatStatusDotColor(item.cheatStatus)}`}></div>
          {getCheatStatusText(item.cheatStatus)}
        </div>
      )}

      {/* Content block - different layout for products vs games/categories */}
      <div className="absolute inset-0 flex flex-col justify-end p-4 z-10">
        {/* Для товаров показываем только название и цену снизу */}
        {item.type === 'product' ? (
          <div className="flex flex-row justify-between items-end w-full">
            <h3 className="text-white text-sm font-normal leading-tight drop-shadow-lg mb-1">
              {item.name}
            </h3>
            <div className="flex flex-col items-end">
              <div className="text-slate-300 text-xs font-light drop-shadow-lg opacity-60">
                {currentContent.from}
              </div>
          ...
          /* Для игр и категорий показываем название, количество и цену */
          <div className="flex justify-between items-end">
            <div className="flex flex-col">
              ...
              <span className="text-slate-200 text-xs font-normal leading-tight drop-shadow">
                {item.productCount} {item.type === 'product' ? currentContent.product : currentContent.products}
              </span>
            </div>

            <div className="text-white text-sm font-light text-right drop-shadow-lg opacity-30">
              {item.priceFrom}
            </div>
          </div>
        )}
...
function CatalogSlugContent() {
  ...
  useEffect(() => {
    // Предзагружаем основные данные каталога при монтировании компонента
    if (!catalogData.main && currentLevel === 'main') {
      const preloadData = async () => {
        try {
          const response = await fetch(`/api/catalog?lang=${lang}&_preload=true`, {
            headers: {
              'Cache-Control': 'max-age=300', // Кэш на 5 минут для предзагрузки
            }
          });
  ...
  // Обрабатываем slug параметры при загрузке
  useEffect(() => {
    ...
    if (search) {
      ...
      // /catalog/game/category
      setCurrentLevel('category');
      // Декодируем URL-encoded параметры
      setCurrentGame(decodeURIComponent(slugs[0]));
      setCurrentCategory(decodeURIComponent(slugs[1]));
  ...
  const handleSearch = useCallback(async (query) => {
    ...
    if (searchValue) {
      ...
      // Возвращаемся к текущему уровню без поиска
      if (currentGame && currentCategory) {
        router.push(`/catalog/${encodeURIComponent(currentGame)}/${encodeURIComponent(currentCategory)}`);
      } else if (currentGame) {
        router.push(`/catalog/${encodeURIComponent(currentGame)}`);
      } else {
        router.push('/catalog');
      }
    }
    setIsSearching(false);
  }, [router, currentGame, currentCategory]);

  // Load catalog data from API - базовая функция без поиска
  const loadCatalogDataBase = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/catalog?${params.toString()}&timestamp=${timestamp}`, {
        headers: {
  ...
  }, [currentGame, currentCategory, currentLevel, lang]);

  // Load catalog data from API - с поиском
  const loadCatalogData = useCallback(async () => {
    ...
    try {
      ...
      // Добавляем заголовки для оптимизации кэширования
      const response = await fetch(`/api/catalog?${params.toString()}&timestamp=${timestamp}&cacheKey=${cacheKey}`, {
        headers: {
          'Cache-Control': 'max-age=60', // Кэш на 1 минуту
        }
      });
  ...
  }, [currentGame, currentCategory, searchQuery, currentLevel, lang, cacheKey]);
  ...
  const checkForUpdates = useCallback(async () => {
    try {
      ...
      if (data.success && data.lastUpdate > serverLastUpdate) {
        setServerLastUpdate(data.lastUpdate);
        invalidateCache();
        loadCatalogData();
      }
    } catch (error) {
  ...
  }, [serverLastUpdate, invalidateCache, loadCatalogData]);

  // Load data when level changes
  useEffect(() => {
    
    // Всегда сбрасываем состояние при изменении параметров
    setIsInitialLoad(true);
    setCatalogData({}); // Очищаем старые данные
    
    // Загружаем данные немедленно без задержки для лучшей производительности
    loadCatalogDataBase();
  }, [currentLevel, currentGame, currentCategory, loadCatalogDataBase]);

  // Reload data when language changes (отдельно от других зависимостей)
  useEffect(() => {
    if (lang) {
  ...
  // Check for updates every 5 seconds - ВРЕМЕННО ОТКЛЮЧЕНО
  // useEffect(() => {
  ...
  const getCurrentData = () => {
    ...
    if (currentLevel === 'search') {
      ...
      const primaryKey = `${currentGame}-${currentCategory}`;
      data = catalogData[primaryKey] || [];
      ...
      // Если нет данных по основному ключу, попробуем найти альтернативные ключи
      if (data.length === 0) {
        const alternativeKeys = Object.keys(catalogData).filter(key => 
          key.includes(currentGame) || key === currentGame
        );
        
        for (const key of alternativeKeys) {
          if (catalogData[key] && catalogData[key].length > 0) {
  ...
  const getBreadcrumbs = () => {
    ...
    if (currentLevel === 'game' && currentGame) {
      // Ищем игру в данных main по slug или id
      ...
      if (gameName) {
        breadcrumbs.push(
          { name: gameName, path: `/catalog/${currentGame}`, active: true }
        );
      }
    } else if (currentLevel === 'category' && currentGame && currentCategory) {
      // Ищем игру в данных main по slug или id
      ...
      if (gameName && categoryName) {
        breadcrumbs.push(
          { name: gameName, path: `/catalog/${currentGame}`, active: false },
          { name: categoryName, path: `/catalog/${currentGame}/${currentCategory}`, active: true }
        );
      }
    }
            ...
            {(isLoadingData && isInitialLoad) ? (
              ...
              <div className="text-center py-16">
                <div className="text-slate-400 text-lg mb-4">
                  {currentLevel === 'search' ? currentContent.nothingFound : 'Нет данных для отображения'}
                </div>
                {currentLevel === 'search' && (
                ...
                <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
                  {currentData.map((item) => (
                    <CatalogCard
                      ...
                      getCheatStatusColor={getCheatStatusColor}
                      getCheatStatusDotColor={getCheatStatusDotColor}
                      getCheatStatusText={getCheatStatusText}
                    />
                  ))}
                </div>
              </>
