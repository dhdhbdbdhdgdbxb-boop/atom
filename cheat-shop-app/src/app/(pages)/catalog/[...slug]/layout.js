// Генерация метаданных для SEO (основная английская версия подкатегорий каталога)
export async function generateMetadata({ params }) {
  ...
  if (slugs.length === 2) {
    ...
    canonicalPath = `/catalog/${slugs[0]}/${slugs[1]}`;
  ...
  return {
    ...
    alternates: {
      ...
      languages: {
        'ru': canonicalPath.replace('/catalog', '/ru/catalog'),
        'en': canonicalPath,
...
export default function CatalogSlugLayout({ children }) {
  return children;
}